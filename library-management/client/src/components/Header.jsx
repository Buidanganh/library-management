...
import { Bell, Command, Layers3, Search, ShieldCheck, X } from "lucide-react";
...
function Header({
 ...
 return globalSearchItems.filter((item) => [item.title, item.meta, item.keywords].filter(Boolean).join(" ").toLowerCase().includes(normalizedQuery)).slice(0, 6);
 ...
 <div className="header-command-zone">
 ...
 <button className="header-command-button" type="button" onClick={onOpenCommand}>
 <Command size={16} />
 <span>Command</span>
 <kbd>Ctrl K</kbd>
 </button>
