...
function normalizeText(value) {
 return String(sanitizeVietnameseText(value) || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/đ/g, "d");
}
...
function searchBooks(context, query) {
 ...
 const books = context.books || [];
 if (!words.length) {
 return books.slice().sort((first, second) => Number(second.availableQuantity || 0) - Number(first.availableQuantity || 0)).slice(0, 5);
 }
...
function getReaderRisks(context) {
 ...
 }).filter((reader) => reader.score > 0).sort((first, second) => second.score - first.score).slice(0, 5);
}
...
function LibraryAssistant({ canManageLibrary = false, isAdmin = false, onNavigate }) {
 ...
 setLoading(true);
 try {
 const nextContext = {...(await settleContext(canManageLibrary)),
 ...
 <button type="button" onClick={refreshContext} aria-label="Là m mới dữ liệu">
 <RefreshCw size={15} />
 </button>
