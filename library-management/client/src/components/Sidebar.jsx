...
function Sidebar({
 ...
 const visibleItems = sidebarMenuItems.filter(canUseMenuItem).filter((item) => {
 if (!normalizedQuery) return true;

 const sectionLabel = sidebarSections.find((section) => section.id === item.section)?.label ?? "";
 return [item.label, item.description, sectionLabel, item.shortcut ? `alt ${item.shortcut}` : ""].join(" ").toLowerCase().includes(normalizedQuery);
 });
 const visibleSections = sidebarSections.map((section) => ({...section,
 items: visibleItems.filter((item) => item.section === section.id),
 })).filter((section) => section.items.length > 0);
 const recentItems = recentPages.filter((page) => page !== currentPage).map((page) => sidebarMenuItems.find((item) => item.id === page)).filter(Boolean).filter(canUseMenuItem).slice(0, 3);
 const favoriteItems = favoritePages.map((page) => sidebarMenuItems.find((item) => item.id === page)).filter(Boolean).filter(canUseMenuItem);
 ...
 const tooltip = [item.label, item.description, shortcutLabel].filter(Boolean).join(" - ");
 ...
 {!collapsed && (
 <div>
 <h1 className="h5 mb-0">Library OS</h1>
 <p className="text-white-50 small mb-0">Command Center</p>
 </div>
 )}
 </div>
 ...
 <div className="sidebar-command-footer mt-auto">
 {!collapsed && (
 <div className="sidebar-os-card">
 <span>OS mode</span>
 <strong>{userRole === "admin" ? "Admin control" : canManageLibrary ? "Librarian desk" : "Reader portal"}</strong>
 <small>{isOnline ? "Đồng bộ trực tuyến" : "Đang mất kết nối"}</small>
 </div>
 )}
