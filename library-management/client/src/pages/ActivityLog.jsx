...
function getActivityRiskScore(activity) {
 const type = activity.type || "";
 let score = 12;

 if (type.includes("deleted")) score += 70;
 if (type.includes("locked") || type.includes("role_updated")) score += 58;
 if (type.includes("fine_updated")) score += 46;
 if (type.includes("reservation.updated")) score += 34;
 if (type.includes("bulk")) score += 30;
 if (type.includes("updated")) score += 24;
 if (type.includes("loan.")) score += 16;

 return Math.min(score, 100);
}

function getActivityRiskTone(score) {
...
function matchesIntelligenceFilter(activity, filter) {
 const type = activity.type || "";

 if (filter === "all") return true;
 if (filter === "risk") return getActivityRiskScore(activity) >= 45;
 if (filter === "destructive") return type.includes("deleted") || type.includes("locked");
 if (filter === "bulk") return type.includes("bulk");
 if (filter === "access") return type.includes("locked") || type.includes("unlocked") || type.includes("role_updated");

 return true;
}

function isActivityWithinDays(activity, days) {
...
function getMetadataDiffRows(metadata = {}) {
 ...
 return Array.from(new Set([...Object.keys(before),...Object.keys(after)])).filter((key) => !Object.is(before[key], after[key])).map((key) => ({
 field: key,
 before: before[key],
 after: after[key],
 }));
...
function getMetadataSummaryRows(metadata = {}) {
 return Object.entries(metadata).filter(([key]) => !["before", "after"].includes(key)).map(([key, value]) => ({ key, value }));
}
...
function ActivityLog() {
 ...
 const matchesIntelligence = matchesIntelligenceFilter(activity, intelligenceFilter);
 ...
 return matchesGroup && matchesIntelligence && matchesType && matchesActor && matchesDate && matchesQuery;
 });
 }, [activities, searchQuery, groupFilter, intelligenceFilter, typeFilter, actorFilter, fromDate, toDate]);
 ...
 const riskActivities = activities.map((activity) => ({...activity,
 riskScore: getActivityRiskScore(activity),
 riskTone: getActivityRiskTone(getActivityRiskScore(activity)),
 })).filter((activity) => activity.riskScore >= 45).sort((first, second) => {
 if (second.riskScore !== first.riskScore) return second.riskScore - first.riskScore;
 return new Date(second.createdAt) - new Date(first.createdAt);
 });
 const todayCount = activities.filter((activity) => isActivityWithinDays(activity, 1)).length;
 const weekCount = activities.filter((activity) => isActivityWithinDays(activity, 7)).length;
 const destructiveCount = activities.filter((activity) => matchesIntelligenceFilter(activity, "destructive")).length;
 const accessCount = activities.filter((activity) => matchesIntelligenceFilter(activity, "access")).length;
 const bulkCount = activities.filter((activity) => matchesIntelligenceFilter(activity, "bulk")).length;
 const actorRanks = Array.from(
 activities.reduce((map, activity) => {
 const actor = activity.actor || "Hệ thống";
 map.set(actor, (map.get(actor) || 0) + 1);
 return map;
 }, new Map())
 ).map(([actor, count]) => ({ actor, count })).sort((first, second) => second.count - first.count).slice(0, 4);
 const typeRanks = Array.from(
 activities.reduce((map, activity) => {
 const type = activity.type || "system";
 map.set(type, (map.get(type) || 0) + 1);
 return map;
 }, new Map())
 ).sort((first, second) => second[1] - first[1]);
 const healthScore = Math.max(0, 100 - riskActivities.length * 5 - destructiveCount * 8 - accessCount * 4);
 const topType = typeRanks[0]?.[0] || "";
 ...
 actorRanks,
 healthScore,
 healthTone: healthScore >= 80 ? "success" : healthScore >= 55 ? "warning" : "danger",
 insightText: topType
 ? `Loại hoạt động nổi bật nhất là ${typeLabels[topType] || topType}, chiếm ${typeRanks[0][1]} dòng nhật ký.`
 : "Chưa đủ dữ liệu để sinh insight vận hành.",
 };
 }, [activities]);
 ...
 useEffect(() => {
 setPage(1);
 }, [searchQuery, groupFilter, intelligenceFilter, typeFilter, actorFilter, fromDate, toDate, pageSize]);
 ...
 <section className={`activity-intelligence-center ${intelligence.healthTone}`}>
 <div className="activity-health-card">
 <span className="page-eyebrow">
 <ShieldAlert size={16} />
 Activity Intelligence
 </span>
 <strong>{intelligence.healthScore}%</strong>
 <p>{intelligence.insightText}</p>
 <div className="activity-health-meter" aria-label={`Điểm an toàn nhật ký ${intelligence.healthScore}%`}>
 <span style={{ width: `${intelligence.healthScore}%` }} />
 </div>
 </div>

 <div className="activity-signal-grid">
 {[
 { label: "Hôm nay", value: intelligence.todayCount, detail: "Hoạt động trong 24 giờ", icon: TrendingUp, days: 1, tone: "primary" },
 { label: "7 ngày", value: intelligence.weekCount, detail: "Nhịp vận hành gần đây", icon: History, days: 7, tone: "success" },
 { label: "Rủi ro", value: intelligence.riskActivities.length, detail: "Sự kiện cần rà soát", icon: AlertTriangle, filter: "risk", tone: intelligence.riskActivities.length > 0 ? "danger" : "success" },
 { label: "Phân quyền", value: intelligence.accessCount, detail: "Khóa/mở/phân quyền", icon: LockKeyhole, filter: "access", tone: intelligence.accessCount > 0 ? "warning" : "success" },
 ].map((item) => {
 const Icon = item.icon;

 return (
 <button
 className={`activity-signal-card ${item.tone} ${item.filter && intelligenceFilter === item.filter ? "active" : ""}`}
 type="button"
 key={item.label}
 onClick={() => {
 if (item.days) {
 ...
 const from = new Date();
 from.setDate(today.getDate() - item.days + 1);
 setFromDate(getDateInputValue(from));
 setToDate(getDateInputValue(today));
 setIntelligenceFilter("all");
 } else {
 setIntelligenceFilter(item.filter);
 }
 setTypeFilter("");
 }}
 >
 ...
 <div className="activity-risk-feed">
 <div className="activity-intelligence-header">
 <strong>Risk feed</strong>
 <button type="button" onClick={() => setIntelligenceFilter("risk")}>
 Xem rủi ro
 <ArrowRight size={14} />
 </button>
 </div>
 {intelligence.riskActivities.length === 0 ? (
 <div className="empty-state compact">Chưa có sự kiện rủi ro cần rà soát.</div>
 ) : (
 <div className="activity-risk-list">
 {intelligence.riskActivities.slice(0, 4).map((activity) => (
 <button
 className={`activity-risk-item ${activity.riskTone}`}
 type="button"
 key={activity.id}
 onClick={() => setSelectedActivity(activity)}
 >
 ...
 <div className="activity-actor-board">
 <div className="activity-intelligence-header">
 <strong>Actor nổi bật</strong>
 <span>{intelligence.actorRanks.length} người</span>
 </div>
 <div className="activity-actor-list">
 {intelligence.actorRanks.map((item, index) => (
 <button
 type="button"
 key={item.actor}
 onClick={() => {
 setActorFilter(item.actor);
 setIntelligenceFilter("all");
 }}
 >
 <span>#{index + 1}</span>
 <strong>{item.actor}</strong>
 <small>{item.count} hoạt động</small>
 </button>
 ))}
 {intelligence.actorRanks.length === 0 && <div className="empty-state compact">Chưa có actor nào.</div>}
 </div>
 </div>
 </section>

 <div className="table-card" style={{ marginTop: 24, marginBottom: 24 }}>
 <div className="quick-filter-strip activity-intelligence-filters">
 {Object.entries(intelligenceLabels).map(([key, label]) => (
 <button
 className={`quick-filter-chip ${intelligenceFilter === key ? "active" : ""}`}
 type="button"
 key={key}
 onClick={() => {
 setIntelligenceFilter(key);
 setTypeFilter("");
 }}
 >
 {label}
 ...
 {types.filter((type) => groupFilter === "all" || getActivityGroup(type) === groupFilter).map((type) => (
